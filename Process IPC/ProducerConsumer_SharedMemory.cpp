...
int main() {
    ...
    sem_t *sem_empty = sem_open("/sem_empty", O_CREAT, 0666, 1); // Tracks empty slots
    ...
    if (pid == 0) { // Consumer process
        ...
        // Cleanup: Remove shared memory segment
        ...
        if (sem_unlink("/sem_empty") == -1 || sem_unlink("/sem_full") == -1) {
            perror("sem_unlink");
            exit(EXIT_FAILURE);
        }
    }
